...
// Simple LCG PRNG for kernel use
...
// Allocate a process structure and kernel stack
struct proc*
allocproc(void)
{
  struct proc *p;
...
userinit(void)
{
  ...
  p = allocproc();
  p->tickets = 1; // Ensure tickets initialized for first user process
  initproc = p;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    if(total_tickets > 0) {
      ...
      int winner = lcg_rand() % total_tickets;
      int running_sum = 0;
